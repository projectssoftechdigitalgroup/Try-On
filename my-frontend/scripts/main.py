    ...
    # -------------------------------
    # Clothing Paths (updated for gendered IDs)
    # -------------------------------
    clothing_paths = {
        # MALE
    ...
    def place_cloth(frame, cloth_img, cloth_type, get_point):
        ...
        # Male and female use same overlay rules — only cloth image changes
        if any(tag in cloth_type for tag in ["pant", "jeans", "pajama", "tunic"]):
        ...
        elif any(tag in cloth_type for tag in ["shirt", "polo", "blouse", "kurta"]):
        ...
        elif any(tag in cloth_type for tag in ["full_suit", "sundress", "gown","skirt"]):
            l_sh, r_sh = get_point("l_shoulder"), get_point("r_shoulder")
            l_toe, r_toe = get_point("l_toe"), get_point("r_toe")
